...
async function run() {
  ...
  if (error) {
    ...
    // wait, Supabase JS won't give me column names on empty array. Let's try to insert an invalid row to see the error.
    const { error: insErr } = await supabase.from('members').insert([{ profile_id: '00000000-0000-0000-0000-000000000000' }]);

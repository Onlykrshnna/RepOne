        ...
        <Card className="md:col-span-1 bg-card border-border text-foreground flex flex-col">
          ...
          <CardContent className="flex-1">
            {isDashboardLoading ? (
              ...
              return (
                <div className="bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/20 rounded-lg p-6 text-center space-y-3">
                  <div className="h-10 w-10 rounded-full bg-primary/20 flex items-center justify-center mx-auto">
                    <CreditCard className="h-5 w-5 text-primary" />
                  </div>
                  <div className="text-primary font-bold text-lg">{plan.membership_plans?.name}</div>
                  <div className="text-sm text-muted-foreground">
                  ...
                  {daysLeft !== null && daysLeft !== undefined && daysLeft <= 7 && (
                     <div className="text-xs text-amber-800 dark:text-amber-400 flex items-center justify-center gap-1">
                      ...
                      <TableRow key={payment.id} className="border-border hover:bg-muted/50">
                        ...
                        <TableCell>
                          <Badge variant="outline" className={`uppercase text-[10px] font-semibold tracking-wider
                            ${payment.status === 'approved' ? 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/20' : ''}
                            ${payment.status === 'pending' ? 'bg-amber-500/10 text-amber-800 dark:text-amber-400 border border-amber-500/20' : ''}
                            ${payment.status === 'rejected' ? 'bg-destructive/10 text-destructive border border-destructive/20' : ''}
                          `}>
                            {payment.status}
                          </Badge>
                        </TableCell>
